...
![decelerate_waypoints_code](imgs/decelerate_waypoints_code.JPG)  
...
![decelerate_waypoints_graph](imgs/decelerate_waypoints_graph.JPG)
...
The reson we need the TFRecord files is to convert from anything that will generate data (say, Pascal VOC format) to TFRecord so we could use them with the Object Detection API.
...
Steps:
- Go to tensroflow/models/object_detection/samples/configs on GitHub
...
- Run your model using the pytho command while including the path to save the model, pipeline for the config file. 
